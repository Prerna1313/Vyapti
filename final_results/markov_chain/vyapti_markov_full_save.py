#!/usr/bin/env python3
"""
Vyapti - Markov Chain (FULL METRICS SAVE)
"""
...
# MARKOV SIMPLE
# =============================================================================
class MarkovSimpleScheduler(BaseScheduler):
    def __init__(self, band_count: int, p_stay: float = 0.3):
        super().__init__(band_count, provenance_note=f"Markov Simple (p_stay={p_stay})")
    ...
    def update(self, action: int, observation: dict):
        ...
        if hit:
            if self.rng.random() < self.p_stay:
                ...
                self.current_band = self.rng.randint(0, self.band_count)
        else:
            self.current_band = self.rng.randint(0, self.band_count)
...
# MARKOV LEARNED
# =============================================================================
class MarkovLearnedScheduler(BaseScheduler):
    def __init__(self, band_count: int):
        super().__init__(band_count, provenance_note="Markov Learned")
        self.band_count = band_count
        self.rng = np.random.RandomState()
    ...
    def update(self, action: int, observation: dict):
        ...
        probs = (self.hit_count + 1) / (self.visit_count + 2)
        probs = probs / probs.sum()
        
        self.current_band = self.rng.choice(self.band_count, p=probs)

# =============================================================================
# RUN
# =============================================================================
print("="*80)
print("VYAPTI - MARKOV CHAIN (FULL SAVE)")
print("="*80)
...
for name, Scheduler in [("Markov_Simple", MarkovSimpleScheduler), ("Markov_Learned", MarkovLearnedScheduler)]:
    print(f"\n{'='*80}\nRUNNING {name}\n{'='*80}")
