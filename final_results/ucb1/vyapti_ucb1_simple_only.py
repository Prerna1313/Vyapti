#!/usr/bin/env python3
"""
Vyapti - SIMPLE UCB1 ONLY
No tuning, no comparison, just classic UCB1
"""
...
class UCB1Scheduler(BaseScheduler):
    def __init__(self, band_count: int, c: float = 1.414):
    ...
    def select_action(self, observation_history, current_time_slot: int) -> int:
        ...
        # UCB1 score: mean + c * sqrt(ln(t) / n)
        mean = self.hit_count / self.visit_count
        confidence = self.c * np.sqrt(np.log(self.t + 1) / self.visit_count)
        scores = mean + confidence
...
for ep_idx, test_file in enumerate(test_files):
    ...
    schedulers = {"UCB1": UCB1Scheduler(band_count=BAND_COUNT)}
    results = run_paired_episodes(env=env, schedulers=schedulers, seed=ep_idx)
    trajectory = results["UCB1"].trajectory
    
    metrics = metrics_engine.record_result(
        episode_id=ep_idx, seed=ep_idx, scheduler_name="UCB1",
...
print(f"  UCB1 Pd:  {pd_mean:.3f} ± {pd_std:.3f}")

if pd_mean > 0.085:
    improvement = (pd_mean - 0.085) / 0.085 * 100
    print(f"  ✓ BEATS RR by +{improvement:.1f}%")
else:
    print(f"  ✗ Worse than RR")
...
all_file = output_dir / "ucb1_simple_all_episodes_all_metrics.json"
...
agg_file = output_dir / "ucb1_simple_aggregate.json"
