...
def get_emitters() -> List[Dict[str, Any]]:
    """
    Returns ALL radar emitters extracted from all TSRD configs:
      - config_0 (original 72-emitter baseline)
      - Folder 3 scan configs (config_1, config_106, config_115, config_124,
        config_160, config_214, config_216, config_223)
      - Folder 4 stare configs (stare_config_1, stare_config_106, etc.)

    Tries the merged tsrd_all_emitters.json first (generated by both
    extract_folder3_data.py and extract_folder4_data.py), then falls back to
    the original 72-emitter JSON so the endpoint never returns an empty list.
    """
    import json
    from pathlib import Path
    lib_dir = Path(__file__).resolve().parent.parent / "lib"
    # Prefer merged all-emitters file (scan + stare)
    all_path = lib_dir / "tsrd_all_emitters.json"
    if all_path.is_file():
        try:
            ...
            pass
    # Fallback: original 72-emitter file
    orig_path = lib_dir / "tsrd_emitters_72.json"
...
@app.get("/api/dataset/configs")
def get_dataset_configs() -> Dict[str, Any]:
    """
    Returns metadata for ALL loaded TSRD scenario configs — both scan (folder 3)
    and stare (folder 4).

    Sources:
      - lib/tsrd_folder3_configs.json  (config_0 + 8 scan configs from folder 3)
      - lib/tsrd_folder4_configs.json  (6 stare configs from folder 4)

    Each config entry carries a `dataMode` field: "scan" or "stare".
    Scan configs without an explicit dataMode are tagged "scan".
    """
    import json
    from pathlib import Path
    from dataset import is_dataset_loaded, dataset_size, dataset_config_stats

    lib_dir = Path(__file__).resolve().parent.parent / "lib"
    configs_meta: list = []

    # --- Folder 3: scan configs (config_0 + 8 scan configs) ---
    scan_json = lib_dir / "tsrd_folder3_configs.json"
    if scan_json.is_file():
        try:
            with scan_json.open() as f:
                scan_configs = json.load(f)
            # Back-fill dataMode for scan configs that pre-date the field
            for c in scan_configs:
                c.setdefault("dataMode", "scan")
            configs_meta.extend(scan_configs)
        except Exception:
            pass

    # --- Folder 4: stare configs ---
    stare_json = lib_dir / "tsrd_folder4_configs.json"
    if stare_json.is_file():
        try:
            with stare_json.open() as f:
                stare_configs = json.load(f)
            for c in stare_configs:
                c.setdefault("dataMode", "stare")
            configs_meta.extend(stare_configs)
        except Exception:
            pass

    scan_count  = sum(1 for c in configs_meta if c.get("dataMode") == "scan")
    stare_count = sum(1 for c in configs_meta if c.get("dataMode") == "stare")

    return {
        "configs": configs_meta,
        "totalConfigs": len(configs_meta),
        "totalPulses": sum(c.get("pulseCount", 0) for c in configs_meta),
        "totalEmitters": sum(c.get("txCount", 0) for c in configs_meta),
        "scanConfigCount": scan_count,
        "stareConfigCount": stare_count,
        "datasetLoaded": is_dataset_loaded(),
        "episodesAvailable": dataset_size(),
        "scanStats": dataset_config_stats(),
    }


@app.get("/api/dataset/config/{config_id}")
def get_config_emitters(config_id: str) -> Dict[str, Any]:
    """
    Returns detailed emitter list for a specific TSRD config
    (e.g. config_1, config_106, config_0, ...).
    """
    import json
    from pathlib import Path
    lib_dir = Path(__file__).resolve().parent.parent / "lib"

    # Load all emitters and filter by configId
    all_path = lib_dir / "tsrd_all_emitters.json"
    if all_path.is_file():
        try:
            with all_path.open() as f:
                all_emitters = json.load(f)
            filtered = [e for e in all_emitters if e.get("configId") == config_id]
            if filtered:
                return {"configId": config_id, "emitters": filtered, "count": len(filtered)}
        except Exception:
            pass

    # Fallback for config_0: original 72-emitter file
    if config_id == "config_0":
        ...
        if orig_path.is_file():
            try:
                with orig_path.open() as f:
                    data = json.load(f)
                return {"configId": config_id, "emitters": data, "count": len(data)}
            except Exception:
                pass
...
def get_environment_matrix(
    offset: int = 0,
    window: int = 60,
    config_id: str = "config_0",
) -> Dict[str, Any]:
    """
    ...
    config_id selects which TSRD config's observation matrix to use:
      - "config_0"       — original 72-emitter baseline (SIH_DATA/2/)
      - "config_*"       — folder-3 scan configs (SIH_DATA/3/)
      - "stare_config_*" — folder-4 stare configs (SIH_DATA/4/)
    """
    from pathlib import Path
    import numpy as np
    ...
    # Determine which observation matrix to load.
    # Routing:
    #   config_0           → SIH_DATA/2/TSRD_READY/ (original baseline)
    #   config_*           → SIH_DATA/3/  (folder 3 scan configs)
    #   stare_config_*     → SIH_DATA/4/  (folder 4 stare configs)
    if config_id == "config_0":
        obs_path = base / "SIH_DATA" / "2" / "TSRD_READY" / "observation_matrix.npy"
    elif config_id.startswith("stare_"):
        # Folder 4 stare configs — uses stare_ prefix in filename
        obs_path = base / "SIH_DATA" / "4" / f"{config_id}_obs_matrix.npy"
    else:
        # Folder 3 scan configs
        obs_path = base / "SIH_DATA" / "3" / f"{config_id}_obs_matrix.npy"
...
# --- SYSTEM C EXTENSIONS ---
from . import system_c_runner

@app.post("/api/system-c/start")
async def system_c_start():
    await system_c_runner.start_system_c()
    return {"status": "started"}

@app.post("/api/system-c/stop")
async def system_c_stop():
    await system_c_runner.stop_system_c()
    return {"status": "stopped"}

@app.websocket("/ws/system-c")
async def websocket_system_c(websocket: WebSocket):
    await websocket.accept()
    try:
        while True:
            message = await system_c_runner.stream_queue.get()
            await websocket.send_json(message)
    except WebSocketDisconnect:
        pass
